...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <div style="padding: 30px; background: #ffffff;">
          ...
          <div style="text-align: center; margin-top: 30px;">
            <a href="${Deno.env.get('SUPABASE_URL')?.replace('.supabase.co', '.lovable.app')}/admin/inventory" 
               style="display: inline-block; background: #D4AF37; color: #000; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: bold;">
              Manage Inventory
            </a>
          </div>
        </div>
    ...
    await resend.emails.send({
      ...
      to: [adminEmail],
      subject: `🚨 Inventory Alert: ${(outOfStockProducts?.length || 0) + (criticalProducts?.length || 0) + (lowStockProducts?.length || 0)} Products Need Attention`,
      html: emailHtml,
    });
